...
public class Execution : BattleState
{
    ...
    /// <summary>
    /// 반응 애니메이션이 끝날 때 실행되는 함수
    /// </summary>
    void ActionGetEnd()
    {
        manager.ActionManager.ActionExecute();                                              // 실제 행동에 따른 데이터 처리
        GameManager.Instance.CoroutineManager.OnChangeState(1.5f, typeof(StateUpdate));     // 1.5초 딜레이 후 갱신 상태로
    }
}
