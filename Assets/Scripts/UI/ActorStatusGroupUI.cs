...
public class ActorStatusGroupUI : MonoBehaviour
{
    ...
    {
        int count = 0;
        BattleSlot[] temp = GameManager.Instance.SlotController.AllySlot;
        ...
        temp = GameManager.Instance.SlotController.AllyStandbySlot;
        ...
        temp = GameManager.Instance.SlotController.EnemySlot;
        for (int i = count; i < temp.Length + count; ++i)
        {
            if (!temp[i].IsEmpty)
            {
        ...
        temp = GameManager.Instance.SlotController.EnemyStandbySlot;
        for (int i = count; i < temp.Length + count; ++i)
        {
            if (!temp[i].IsEmpty)
            {
