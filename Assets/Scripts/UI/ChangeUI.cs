...
public class ChangeUI : MonoBehaviour
{
    ...
    void OnSwap()
    {
        if (slot != null)
        {
            BattleSlot temp = GameManager.Instance.BattleManager.OnTurnSlot;
            GameManager.Instance.SlotVisualizer.SwapSlot(temp, slot); 
            GameManager.Instance.BattleManager.SwapCharacter(slot as StandbySlot);
            //SetSlot(temp);
            GetData();
        }
    }
